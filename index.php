...
for ($i=0;$i<sizeof($a);$i++)
{
	...
	$q = "select distinct secteur_groupe from topo_secteur where site_id=".$a[$i]['site_id']." and secteur_groupe !='' order by secteur_ordre";
   ...
   <head>
    ...
    <base href="<?=$config["BaseUrl"]?>"/>
 	...
 	<meta name="description" content="Mise à disposition gratuitement de topo d'escalade numérique. Avec la possibilité de commenter les sites, secteurs et voies." />
        ...
        var initHash = '<?=@ereg_replace('/','-',substr($_SERVER['REQUEST_URI'],1))?>';
...
else
{
 ...
 <!-- HTML5 shim and Respond.js IE8 support of HTML5 elements and media queries -->
    ...
    <![endif]-->
    <link  href="<?=$config["BaseUrl"]?>css/rtg2.css" rel="stylesheet">
      ...
      <nav id="menu">
         <ul role="menu">
		   ...
		   <li><span class="glyphicon fa fa-map-marker">&#160;&#160;Les sites</span>
		    		<ul >
					<?php
					while(list($type,$list) = each($liste_sites_menu))
					{
