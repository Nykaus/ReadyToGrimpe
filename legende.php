...
<div class="panel panel-default legende">
			...
			<div class="panel-body">
				...
				while (list($piK,$PIv) = each($config['PI']))
				{
...
<?php 
reset($config['type']);
while (list($tk,$type) = each($config['type']))
{
?>
			<div class="panel panel-default legende">
			...
			<div class="panel-body">
					<?php
						while (list($vk,$val) = each($type['valeurs']))
						{
							echo '<div class="item"><img src="img/'.$tk.'_'.strtolower (@ereg_replace("[[:blank:]]","",$val)).'.png">'.$val.'</div>';
						}
					?>
			</div>
			</div>
